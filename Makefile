...
test-coverage: fmt test-coverage-tools | $(BASE) ; $(info $(M) running coverage tests…) @ ## Run coverage tests
...
vendor: $(BASE) ; $(info $(M) updating dependencies…)
	$Q cd $(BASE) && $(DEP) ensure -v
	@ln -nsf . vendor/src
	@touch $@

run: $(BASE) ; $(info $(M) Starting local server…)
	cd $(BASE) && $(GO) run main.go
...
migrate: $(BASE) ; $(info $(M) Building goose binary…)
	cd $(BASE) && $(GO) build -o goose cmd/goose.go && ./goose $(migrateargs)
